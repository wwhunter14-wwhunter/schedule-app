...
export default async function SchedulesPage({
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 fade-in">
      ...
      <div className="mt-4 text-sm text-slate-500 dark:text-slate-400 mb-3">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                  {format(s.startAt, 'yyyy.MM.dd')} {s.allDay ? '(하루 종일)' : format(s.startAt, 'HH:mm')}
                </p>
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  {s.category && <CategoryBadge name={s.category.name} color={s.category.color} />}
                  {s.tags.map(({ tag }) => (
                    <span key={tag.id} className="text-xs px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                      #{tag.name}
                    </span>
                  ))}
                </div>
