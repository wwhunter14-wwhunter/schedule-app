    ...
    <div className="max-w-3xl mx-auto px-4 py-8 fade-in">
      ...
      {schedules.length === 0 ? (
        ...
        <div className="space-y-2">
          {schedules.map((s) => (
            <Link key={s.id} href={`/schedules/${s.id}`} className="flex items-start gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md transition-all">
              ...
              <svg className="w-4 h-4 text-slate-300 dark:text-slate-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            </Link>
          ))}
        </div>
      )}
